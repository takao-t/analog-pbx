...
IP接続ユニット(IP-UNIT)を開発し、対応しました。

## これは何？

アナログPBXを開発するためのハード/ソフト一式です。必要な機能をそれぞれコンポーネント化することにより、お好みの方法でアナログPBXをつくることができます。

電話機そのものを制御する部分(いわゆる加入者回線部分)であるSLICユニット、音声の繋ぎ替えを行う交換台であるスイッチボードユニット、電話交換の機能そのものを行うPBXコアの3つのユニットを提供しています。全部あわせて使ってもかまいませんし、必要な部分だけ使ってもかまいません。
...
さらに理由を付け加えると生成AIの登場です。PBXのようなプロジェクトは、その仕組みと何を実装すれば良いのかがわかってはいても、いざコードを書こうとすると、かなりの重労働です。しかも自分が書いた信用できないコードをデバッグしていると、あっという間に数年の時間が過ぎることでしょう。AsteriskベースのIP-PBXなんて気付けば10年も書いてました。バカですね。
...
SLICユニットの制御は5ピンのデジタルI/O(ロジック)またはシリアル(UART)で行います。SLICユニット単体でトーン生成、ベル鳴動、ケイデンス制御に加えてDTMF-DP変換を行います。
...
※ちょっと難点な部分を解消するため、2x4のスイッチボードも開発・公開しました。SB_2X2が2X2のスイッチボード、SB_2X4が2X4のスイッチボードです。

スイッチボードの制御はシリアル(UART) 1本で行います。どのスイッチを閉じる/開くかをコマンドで送出するだけです。複数ユニットを接続する場合にはシリアルを順次送りで繋いで増やします。
...
PBXをIP接続するためのユニットです。SLICユニットと同等に扱うことができます。ただしIP専用の設定と発信のためのプレフィクス設定が必要となります。

SIP等の処理部分は外部のSBC(Single Board Computer : Raspberry Piなど)に頼ります。このユニットはPBXの処理を外部に引き出すためのユニットで、SBCとはUARTモードで接続を行います。

SLICユニットとは排他の関係にあり、例えば4回線PBXを構成した場合、SLIC×3,IP×1のような組み合わせになるのでPBX本体の回線容量を越えることはできませんので注意してください。
...
4回線用PBXの場合にはケースも用意していますので、ご利用ください。モジュラージャックには「よく売っている」リード付のスロット装着タイプが使えます。電源入力(ACアダプタ接続)部分には丸穴取り付けタイプのDCジャックが使えるはずです。回り防止に2か所、平たいところがあると思いますが、サポート無で出力すると丸穴がタレるので、それを利用します。
...
もしこのPBXを使って『本物』の電話機を実働展示される場合には、『この電話機は内線専用です』のような文言での注意書きを掲示してください。普通の電話機が置かれていると、その電話は使えると判断され、非常時に緊急番号にダイヤルしようとする可能性があるからです。固定電話を見る機会そのものが減ってる昨今なので、もはやこんな心配をしなくてもいいのかもしれませんが、念のため注意書きしておいてください。
...
## プロトコル
各ユニット間の主な流れは以下のようになっています。

### SLICユニット-PBXコア
基本4or5ピンI/O(オプションでUARTモードもあり)

主な処理の流れ

|電話機|-|SLICユニット|-|PBXコア|備考|
|-----|-----|-----|-----|-----|-----|
|オンフック|→|オンフック検出(HO=L)|→|何もしないor切断処理||
|オフフック|→|オフフック検出(HO=H)|→|オフフック検出||
|||CT生成|←|CT指示(T1=L,T2=L)||
|発信音確認|||||
|ダイヤル|→|オン/オフフック(HO=パルス)|→|ダイヤルパルス検出||
|||CT停止|←|トーン停止指示(T1=H,T2=H)||
|||||ダイヤル完了検出||
|||RBT生成|←|RBT指示(T1=H,T2=L)||
|リングバック音聴取||||||
|||||ルーティング開始||
|||||相手先電話機に鳴動指示(RI=L)|→相手先電話機へ|
|||||接続完了(相手先のOH検出)||
|||||鳴動停止(RI=H)|→相手先電話機鳴動停止|
|||RBT停止|←|トーン停止(T1=H,T2=H)||
|||||スイッチボードへ接続指示||
|通話|-|-|-|HO状態監視|相手先電話機と通話|
...
UART(9600bps)でのシリアルコマンド。終端は0x0d。

例: 内線ポート1と内線ポート3を接続する場合

"C0103"と"C0301"を送出

クロスポイントスイッチでそれぞれの縦横をオーディオ入力、オーディオ出力とするため『1のインを3のアウト』『3のインを1のアウト』と2ポイント接続する必要があるため。オーディオはSLICモジュールによりイン/アウトが分離されている。

切断する場合には

"R0103"と"R0301"を送出

PBXコアからスイッチボードへのコマンドはシリアル1本のみで行う。
